...
interface SingleUpgradeProps { upgradeKey: UpgradeKeys }
export function SingleUpgrade({ upgradeKey }: SingleUpgradeProps) {
  ...
  const Upgrade = useMemo(()=>new UpgradeModel(upgrade, count),[upgradeKey,count])
  return (<div key={upgradeKey} style={{ border: "1px solid black" }} onClick={() => { attemptUpgrade(upgradeKey) }}>
    <p>
      {upgradeKey}: {JSON.stringify(upgrade)}
    </p>
    <p>Cost: {JSON.stringify(Upgrade.getCost())}</p>
    <p>
      Can Purchase: {JSON.stringify(Upgrade.canPurchase(resources))}
    </p>
    <button onClick={() => { dispatch(attemptUpgrade(upgradeKey)) }}>
      buy
    </button>
  </div>)
}
